...
func main() {
	log := logger.WithService(logger.New(os.Getenv("LOG_LEVEL")), "contact")

	cfg := &conf.Server{
		GRPC: conf.GRPCServer{Addr: envOrDefault("Contact_GRPC_ADDR", ":9103")},
	}
	dataCfg := &conf.Data{}
	...
	go func() {
		log.Info("contact service starting", "addr", addr)
