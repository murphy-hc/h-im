...
func main() {
	log := logger.WithService(logger.New(os.Getenv("LOG_LEVEL")), "media")

	cfg := &conf.Server{
		GRPC: conf.GRPCServer{Addr: envOrDefault("Media_GRPC_ADDR", ":9107")},
	}
	dataCfg := &conf.Data{}
	...
	go func() {
		log.Info("media service starting", "addr", addr)
