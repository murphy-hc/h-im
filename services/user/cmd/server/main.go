...
func main() {
	log := logger.WithService(logger.New(os.Getenv("LOG_LEVEL")), "user")

	cfg := &conf.Server{
		GRPC: conf.GRPCServer{Addr: envOrDefault("User_GRPC_ADDR", ":9101")},
	}
	dataCfg := &conf.Data{}
	...
	go func() {
		log.Info("user service starting", "addr", addr)
